...
import com.rudyrachman16.mtcatalogue.databinding.FragmentTabBinding
import com.rudyrachman16.mtcatalogue.views.detail.DetailActivity
...
class TabFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (position != null) {
            ...
            when (position) {
                0 -> {
                    val rvAdapter = HomeAdapter<Movie>({ number, title, rating, voter ->
                        HomeAdapter.share(requireActivity(), position, number, title, rating, voter)
                    }) { it, number ->
                        startActivity(Intent(requireContext(), DetailActivity::class.java).apply {
                            putExtra(DetailActivity.KEY_MOV, it)
                            putExtra(DetailActivity.KEY_POS, position)
                            putExtra(DetailActivity.KEY_NUM, number)
                        })
                    }
                    rvAdapter.setList(viewModel.getMovies())
                    makeRV(rvAdapter)
                }
                1 -> {
                    val rvAdapter = HomeAdapter<TvShow>({ number, title, rating, voter ->
                        HomeAdapter.share(requireActivity(), position, number, title, rating, voter)
                    }) { it, number ->
                        startActivity(Intent(requireContext(), DetailActivity::class.java).apply {
                            putExtra(DetailActivity.KEY_TV, it)
                            putExtra(DetailActivity.KEY_POS, position)
                            putExtra(DetailActivity.KEY_NUM, number)
                        })
                    }
                    rvAdapter.setList(viewModel.getTvShow())
                    makeRV(rvAdapter)
                }
            }
        }
